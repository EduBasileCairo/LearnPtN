...
# EJEMPLO 3. RECICLAJE
# Eres un conductor de camión de reciclaje. Le gustaría recopilar algunas estadísticas sobre cuánto recicla cada casa. Suponga que los números de las casas son 0, 1, 2, 3...Cada casa puso su reciclaje en cajas. Calcula el número de la casa con la mayor cantidad de reciclaje y la cantidad de cajas que recicla la casa. Lo mismo para la casa con menos y el número de cajas para esa casa.
...
# Returns the index with the largest value in the list and the number of crates for that house. Raises ValueError if list is empty.
...
# Returns the smallest value in the list and a list of house number (list indexes) with that value. Raises ValueError if list is None or empty.
    if crates is None or len(crates) == 0:
        raise ValueError('A list with at least one element is required')
    min_houses = []
    min_crates = crates[0]
